...
class Adafruit_Si7021 {
 public:
  ...
  /*!
  Added Juraj Repcik
  @brief  RH conversion triggers also Temperature conversion, 
  conversion time for 14b Temp + 12b RH is typ 17ms (max 22.8ms). 
  Request measurement with function .RHrequest_measurement. 
  Use .read_requested for reading.
  @return read error (0=error)
  */
  uint8_t RHrequest_measurement();
    /*!
  Added Juraj Repcik
  @brief  .read_requested function reads previously requested Temperature and RH. Function must be called at least 23ms after RHrequest_measurement. (typ 17ms)
  @param  t var. address, temperature reading 14bit
  @param  rh  var. address, relative humidity reading 12bit
  @return read error (0=error)
  */
  uint8_t read_requested(uint16_t *t,uint16_t *rh);
