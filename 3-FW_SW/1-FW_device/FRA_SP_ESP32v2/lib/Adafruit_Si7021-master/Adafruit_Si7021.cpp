...
uint8_t Adafruit_Si7021::RHrequest_measurement(){
  _wire->beginTransmission(_i2caddr);
...
uint8_t Adafruit_Si7021::read_requested(uint16_t *t,uint16_t *rh){
  //A temperature measurement will always return XXXXXX00 in the LSB field
  *t = 0b11; //filling wrong value
  //A humidity measurement will always return XXXXXX10 in the LSB field
  *rh = 0b11; //wrong value for check 
  if (_wire->requestFrom(_i2caddr, 3) == 3) {
    *rh = _wire->read() << 8 | _wire->read();
    uint8_t chxsum = _wire->read();
    /*  conversion done in labview
    float humidity = *rh;
    humidity *= 125;
    humidity /= 65536;
    humidity -= 6;
    */
    ...
    return 0; //error reading sensor, did you wait enough from request?
  }
  _wire->beginTransmission(_i2caddr);

  _wire->write(SI7021_READPREVTEMP_CMD);//temperature that was measured when RH measurement requested, answer is immediate
  uint8_t err = _wire->endTransmission();
  if (err != 0)
    return 0; //error
