...
//NDIR module custom lib
#include "NDIR_module.h"

//debug functions
//#define DEBUG_FX_ACTIVE
#ifdef DEBUG_FX_ACTIVE
  #include "debug_fx.h"
#endif
...
bool use_NDIR_module=1;//Temperature and RH sensor
...
void setup(){
  ...
  //fast check if NDIR is present
  if (NDIR_connection_check(NDIR_ADDR)) {
    log_error_code(43);
    error("NDIR module init error\n");
    use_NDIR_module = 0;
  } else {
    uint32_t start_NDIR = millis();

    //test read and error reporting
    uint16_t NDIRstatus = 0xFFFF;
    uint16_t NDIRgas = 0;
    uint16_t NDIRth = 0;
    PRINTDEBUG("Waiting for NDIR module to get first data...(max 2.5s)\n");
    for(uint8_t i=0;i<50;i++){ //50*50ms = max 2.5s
      delay(50);
      if (readNDIRRegisters(NDIR_ADDR, &NDIRstatus, &NDIRgas, &NDIRth)){
        log_error_code(43);
        error("NDIR module not found - continue without\n");
        use_NDIR_module = false;
        break;
      }
      //PRINTDEBUG("NDIR %d Status 0x%04X\n",i,NDIRstatus);
      if(NDIRstatus & 0x000E){ //0b xxxx xxxx xxxx xxxx xxxx 1110
        //not ready, continue looping.
      } else {
        //no error bit in status
        PRINTDEBUG("NDIR status is ready in %ld ms, Status 0x%04X\n",millis()-start_NDIR,NDIRstatus);
        break;
      }
    }
    //check if valid
    if(NDIRstatus == 0xFFFF && NDIRgas == 0xFFFF && NDIRth == 0xFFFF){
      log_error_code(44);
      error("NDIR module data not valid (all 0xFFFF)\n");
    }
    if(NDIRstatus & 0x0001){
      log_error_code(45);
      error("NDIR module error - common voltage off\n");
    }
    if(NDIRstatus & 0x000E){ //0b xxxx xxxx xxxx xxxx xxxx 1110
      log_error_code(46);
      error("NDIR module error - ACT or REF clipping\n");
    }
    if(NDIRstatus & 0x0010){
      log_error_code(47);
      error("NDIR module error - sensor not detected (temperature <-25 degC)\n");
    }
  }
  #ifdef DEBUG_FX_ACTIVE
    if(use_NDIR_module)debug_NDIR();
  #endif
      ...
      case s12_save_data:
        ...
        if(use_Si7021){
          ...
          if(!Si7021.read_requested(&Sit,&SiRH)){
            use_Si7021 = 0;
            log_error_code(21);
            PRINTDEBUG("Si7021 not reponsing to read requested - did you leave enough time from request?\n");
          }
          ...
          //add new sensor type NDIR and data
        }
        if(use_NDIR_module){
          uint16_t NDIRstatus = 0xFFFF;
          uint16_t NDIRgas = 0;
          ...
          sendUDP_Buffer[++sendUDP_len]=0xF4; //NDIR module gas value
          sendUDP_Buffer[++sendUDP_len]=(NDIRgas >> 8);
          sendUDP_Buffer[++sendUDP_len]=(NDIRgas & 0xFF);
          sendUDP_Buffer[++sendUDP_len]=0xF5; //NDIR module temperature
          sendUDP_Buffer[++sendUDP_len]=(NDIRth >> 8);
          sendUDP_Buffer[++sendUDP_len]=(NDIRth & 0xFF);
          #ifdef DEBUG_FX_ACTIVE
            debug_NDIR_UDP_datagram(sendUDP_Buffer,sendUDP_len);
          #endif
        }
